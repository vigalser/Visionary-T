...
int main(){
  ...
  // SI DA ERROR SE PUEDE QUITAR, PORQUE NO SE COMO IRA EL TEMA DE USAR argc, argv[]
  //if (argc != 2) {
  ...
  namedWindow("Segmentation by ViBe", CV_WINDOW_AUTOSIZE);

  cout << "Introduce the video filename + format. eg: sequence.avi" << endl;
  char* filename;
  cin >> filename;
...
void processVideo (char* videoFilename)
{
  ...
  while ((char)keyboard != 'q' && (char)keyboard != 27) {
    ...
    /* Applying ViBe.
     * If you want to use the color version of ViBe:
     * (1) remplace C1R by C3R in this file.
     * (2) comment the next line (cvtColor).
     */
